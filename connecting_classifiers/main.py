...
tree_entropy = DecisionTreeClassifier(criterion='entropy', random_state=state)
tree_gini = DecisionTreeClassifier(random_state=state)
tree_entropy.fit(X_train, y_train)
tree_gini.fit(X_train, y_train)

forest = RandomForestClassifier(n_estimators=100, random_state=state)
forest.fit(X_train, y_train)

logistic_regression = LogisticRegression(max_iter=1000, random_state=state)
svm = SVC(random_state=state, probability=True)
...
voting_clf_2 = VotingClassifier(estimators=[('lr', logistic_regression), ('svm', svm), ('rf', forest)], voting='soft')
